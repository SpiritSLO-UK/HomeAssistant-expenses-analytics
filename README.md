...
| - | **v1.1.0** (on `main`, next release) - a customisable grouped sidebar (rename/hide/reorder/regroup, per user) with sub-tabbed group and Settings pages, forecasts (budget pace, project burn-down, savings time-to-goal), edit-after-creation for projects/budgets/rules/savings goals, an app-wide date format, a dedicated Tags page, export the selected/filtered transactions, MFA backup codes, opt-in security-event MQTT notifications, background cloud-AI batches with live progress, search filter tokens + keyboard nav, audit-log search/filters + CSV export, in-app dialogs, optimistic selects, AI abuse guards, a hardened non-root container, and a Playwright e2e suite | ✅ |
