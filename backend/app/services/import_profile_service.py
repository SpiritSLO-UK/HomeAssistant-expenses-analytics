...
def _to_out(p: ImportProfile) -> dict:
    try:
        ...
        mapping = {}
    return {"id": p.id, "name": p.name, "mapping": mapping, "default_currency": p.default_currency}
...
def create_profile(db: Session, *, name: str, mapping: dict[str, str], default_currency: str = "GBP") -> dict:
