...
@router.get("")
def list_assets(kind: str | None = None, db: Session = Depends(get_db)) -> list[dict]:
...
def update_asset(asset_id: int, payload: AssetUpdate, db: Session = Depends(get_db)) -> dict:
    asset = _get(db, asset_id)
    try:
...
def list_logs(asset_id: int, db: Session = Depends(get_db)) -> list[dict]:
    _get(db, asset_id)
    return [asset_service.log_to_dict(lg) for lg in asset_service.list_logs(db, asset_id)]


@router.post("/{asset_id}/logs", status_code=201)
def add_log(asset_id: int, payload: AssetLogCreate, db: Session = Depends(get_db)) -> dict:
    _get(db, asset_id)
    try:
        log = asset_service.add_log(
            db, asset_id, **payload.model_dump(exclude_unset=False)
        )
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
