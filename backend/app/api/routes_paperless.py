...
def status(db: Annotated[Session, Depends(get_db)]) -> dict:
    return paperless_service.status(db)


@router.post("/test", responses={400: {"description": "Bad request"}, 502: {"description": "Upstream error"}})
def test_connection(db: Annotated[Session, Depends(get_db)]) -> dict:
    """Verify the configured URL + token can reach Paperless."""
    try:
        return paperless_service.test_connection(db)
...
def list_documents(
    db: Annotated[Session, Depends(get_db)], query: str | None = None, limit: int = 25
) -> list[dict]:
    try:
