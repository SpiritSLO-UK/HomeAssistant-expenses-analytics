...
def list_rates(db: Session = Depends(get_db)) -> list[FxRate]:
...
def add_rate(payload: ManualRate, db: Session = Depends(get_db)) -> FxRate:
...
@router.get("/missing")
def missing(db: Session = Depends(get_db)) -> dict:
    count = db.scalar(
