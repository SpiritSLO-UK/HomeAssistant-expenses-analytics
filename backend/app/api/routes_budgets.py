...
from app.models import Budget, Category, Project
...
def _validate(db: Session, *, period: str | None, category_id: int | None, project_id: int | None) -> None:
...
def create_budget(payload: BudgetIn, db: Session = Depends(get_db)) -> Budget:
    _validate(db, period=payload.period, category_id=payload.category_id, project_id=payload.project_id)
